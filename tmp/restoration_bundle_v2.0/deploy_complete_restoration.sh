...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help)
            ...
            echo "  --skip-registry             Skip entity registry restoration (safety)"
...
if [[ "$SKIP_REGISTRY" == "true" ]]; then
    echo -e "\033[1;36m🛡️  ENTITY REGISTRY SKIP ENABLED - Preserving current entity registry for safety\033[0m"
    echo -e "\033[0;36m   This will restore infrastructure while keeping your current entities intact.\033[0m"
    echo
fi
...
detect_environment() {
    ...
    RESTORE_BACKUP_DIR="/tmp/complete_restore_backup_$(date +%Y%m%d_%H%M%S)"
...
if [[ "$SKIP_REGISTRY" == "true" ]]; then
    echo -e "⏭️  Registry skip enabled; no read/write to ${STORAGE_DIR} will be attempted."
    echo -e "📁 Complete system backup directory: ${BOLD}$RESTORE_BACKUP_DIR${NC}"
elif [[ "$DRY_RUN" == "false" ]]; then
    echo "🔄 Creating comprehensive backup of current .storage directory..."
    if [[ -r "$STORAGE_DIR" ]] && [[ -x "$(dirname "$STORAGE_DIR")" ]]; then
        echo -e "📦 Backing up current .storage to ${RESTORE_BACKUP_DIR}/current_storage_backup"
        cp -r "$STORAGE_DIR" "${RESTORE_BACKUP_DIR}/current_storage_backup"
        echo -e "${GREEN}✅ Current system backup completed${NC}"
    elif [[ ! -d "$STORAGE_DIR" ]]; then
        echo -e "${YELLOW}⚠️  No existing .storage directory found (expected in catastrophic recovery)${NC}"
    else
        echo -e "${YELLOW}⚠️  Current .storage not accessible from this shell (SSH add-on protection mode)${NC}"
        echo -e "${YELLOW}   This is expected when running with restricted permissions${NC}"
    fi
else
...
get_category_files() {
    case "$1" in
        ...
        "CRITICAL_ENTITIES")
            if [[ "$SKIP_REGISTRY" == "true" ]]; then
                echo ""  # Return empty string when skipping registry
            else
