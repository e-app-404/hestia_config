...
supersedes:
  - ADR-0016  # operator mount path & aliases
  - ADR-0010  # workspace shape & autofs mounts
  - ADR-0012  # workspace taxonomy with ~/hass
amends:
  - ADR-0015  # symlink policy & workspace
  - ADR-0019  # remote topology mirror policy
  - ADR-0014  # recorder/db storage locations
  - ADR-0022  # keychain creds & telemetry paths
superseded_by: null
...
**The canonical and only supported Home Assistant configuration root is `/config`** across all environments (Home Assistant Host/Supervisor/Core, macOS operator workstation, containers, and CI/CD including GitHub Actions). All tools, scripts, documentation, examples, and templates **MUST** reference `/config` as the Home Assistant root. Any other path names (e.g., `~/hass`, `/homeassistant`, `/n/ha`, `/Volumes/HA/Config`, actions-runner workdirs) are considered implementation details and **MUST NOT** appear in application logic or normative documentation.
