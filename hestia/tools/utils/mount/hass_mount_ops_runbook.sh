...
case $choice in
  ...
  2)
    ...
    diskutil unmount "$HOME/hass" || true
  ...
  3)
    ...
    "$HOME/bin/hass_telemetry.sh" && echo "✅ WEBHOOK_OK"
  ...
  6)
    echo "=== HEALTH PROBE ==="
    if [[ -f "$HOME/hass/hestia/config/diagnostics/.last_mount_status.json" ]]; then
      python3 -c "
import json
with open('$HOME/hass/hestia/config/diagnostics/.last_mount_status.json') as f:
    data = json.load(f)
