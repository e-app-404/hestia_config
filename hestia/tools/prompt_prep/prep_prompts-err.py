...
class PromptPrepper:
    ...
    def _extract_title_from_content(self, content, filepath):
        ...
        objective_match = re.search(r'(?:objective|purpose|goal)[:*]\s*(.+?)(?:\n|$)', content, re.IGNORECASE)
        if objective_match:
    ...
    def _detect_tier(self, content, filepath):
        ...
        if any(kw in content_lower for kw in ['governance', 'emergency', 'critical', 'core system', 'meta']):
            return 'α'
        elif any(kw in content_lower for kw in ['integration', 'operational', 'workflow', 'extraction', 'automation']):
            return 'β'
        elif any(kw in content_lower for kw in ['instruction', 'guide', 'template', 'documentation', 'tutorial']):
            return 'γ'
        elif any(kw in content_lower for kw in ['universal', 'meta', 'framework']):
            return 'Ω'
