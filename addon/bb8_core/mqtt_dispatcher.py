...
# Idempotency set for scanner discovery publisher (per-entity unique_id)
_SCANNER_DISCOVERY_PUBLISHED_UIDS: set[str] = set()
...
_DISPATCHER_DISCOVERY_PUBLISHED_UIDS = set()
REASONS = {
    0: "success",
...
def _maybe_publish_bb8_discovery() -> None:
    ...
    for topic, uniq_id, payload in entities:
        if uniq_id in _DISPATCHER_DISCOVERY_PUBLISHED_UIDS:
...
def _apply_pending_subscriptions():
    ...
    Removes successfully bound subscriptions from the pending list.
    """
    if CLIENT is None:
        return
